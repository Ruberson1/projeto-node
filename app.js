...
//Open Route - Public
app.get('/',(req, res) => {
    res.status(200).json({msg: "Bem vindo a sua API"})
})
...
app.post('/auto/register', async(req, res) => {
    ...
    if (userExisits) {
        ...
        return res.status(422).json({msg: 'Já existe um usuário usando este email'})
...
mongoose
.connect(
    `mongodb+srv://${dbUser}:${dbPassword}@cluster0.ey1gb.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`)
.then(() => {
    app.listen(3000)
